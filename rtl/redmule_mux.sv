...
#(
  ...
  always_comb begin : winner_assignment
    ...
    if (in_req[WsourceStreamId]) begin                  // W (highest)
    ...
    end else if (in_req[WExpSourceStreamId]) begin      // W_EXP (low priority)
      winner_d = WExpSourceStreamId;
    end else if (in_req[XExpSourceStreamId]) begin      // X_EXP (lowest priority)
      winner_d = XExpSourceStreamId;
    end
  end
  ...
  for (genvar ii = 0; ii < NB_CHAN; ii++) begin : in_port_binding
    ...
    // We do not have outstanding transactions AND r_valid has to be asserted 1 cycle after the grant
    ...
    assign in[ii].egnt     = in_egnt[ii];
    assign in[ii].r_evalid = in_r_evalid[ii];
  end
...
`ifndef VCS
  for (genvar i = 0; i < NB_CHAN; i++) begin
    initial dw  : assert(in[i].DW  == out.DW);
    initial bw  : assert(in[i].BW  == out.BW);
    initial aw  : assert(in[i].AW  == out.AW);
    initial uw  : assert(in[i].UW  == out.UW);
    // initial iw_in : assert(in[i].IW == 0);
    initial iw_out : assert(out.IW >= $clog2(NB_CHAN));
    initial ew  : assert(in[i].EW  == out.EW);
    initial ehw : assert(in[i].EHW == out.EHW);
  end
